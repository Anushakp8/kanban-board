...
import { COLOR_PRESETS, CreateTeamMemberSchema } from '../../types';
...
export function TeamMembersPanel({ onClose }: Props) {
  const { teamMembers, addTeamMember, removeTeamMember } = useBoard();
  ...
  const handleAdd = async () => {
    setError('');
    const parsed = CreateTeamMemberSchema.safeParse({ name: name.trim(), color });
    if (!parsed.success) {
      setError(parsed.error.issues[0]?.message ?? 'Invalid input');
      return;
    }
    const success = await addTeamMember(parsed.data.name, parsed.data.color);
    if (success) {
      setName('');
      setShowForm(false);
    } else {
      ...
      <DialogContent className="!pt-0">
        ...
        <div className="space-y-2 mb-4">
          {teamMembers.map(member => (
            <div key={member.id} className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-slate-50 group transition-colors">
              <div
